...
class FeedQueryServiceTest extends Specification {
    FeedQueryService postQueryService
    FeedRepository postRepository = Mock()

    void setup(){
        postQueryService = new FeedQueryService(postRepository)
    }
    ...
    def "List<Post>를 PageResult<GetBlogPostsResponse>로 적절히 변환한다"() {
        ...
        def mockPosts = [
                new Feed("ex_title1","ex_description1", LocalDateTime.now(),givenUser),
                new Feed("ex_title2","ex_description2", LocalDateTime.now().minusMinutes(10),givenUser),
                new Feed("ex_title3","ex_description3", LocalDateTime.now().minusMinutes(20),givenUser)
        ]
